#!/usr/bin/env python3
"""Generate multi-spec HTML tables from regression results."""
...
def read_results(mode: str, outcome: str) -> pd.DataFrame:
    ...
    df.sort_values(["flow", "partner", "base_year", "period_order", "iv_label"], inplace=True)
    return df
...
def render_html(mode: str, outcome: str, grouped: list[tuple[str, str, pd.DataFrame]],
    ...
    for idx, (flow, partner, table_df) in enumerate(grouped, start=1):
        spec_id = f"Spec {idx:02d}"
        parts.append(f"<div class='spec-block'>")
        parts.append(f"<h2>{spec_id}: flow = {flow}, partner = {partner}</h2>")
        parts.append(f"<p class='meta'>Start of {spec_id}</p>")
        html_table = table_df.to_html(index=False, escape=False, border=0)
...
def main():
    ...
    seen = set()
    for _, row in df.iterrows():
        key = (row.flow, row.partner)
        if key in seen:
            continue
        group_df = df[(df.flow == row.flow) & (df.partner == row.partner)]
        if group_df.empty:
            continue
        groups.append((row.flow, row.partner, build_table(group_df)))
        seen.add(key)
